...
def get_movie_recommendations(df, num_recommendations=5):
    ...
    for index, row in df.iterrows():
        ...
        is_suitable = llm_inference(prompt)
        
        # Append the result only if the movie title hasn't been seen before
        if movie_title not in seen_titles:
            results.append({
                'movie_title': movie_title,
                'recommended_for_children': 'Yes' if is_suitable.strip() == "Yes" else 'No'
            })
            seen_titles.add(movie_title)  # Mark this title as seen

    # Convert results to DataFrame for better visualization
    results_df = pd.DataFrame(results)  # Create DataFrame from results list
...
# Limit to the first 500 rows for LLM inference
limited_df = deduplicated_df.head(500)

df = pd.DataFrame(limited_df)

# Get movie analysis
analysis_result = get_movie_recommendations(df, num_recommendations=5)
