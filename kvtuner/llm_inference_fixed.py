...
# Import tqdm normally (we'll use environment variables to control vLLM's tqdm usage)
...
# Add paths for vLLM and KVTuner - Updated for user's environment
...
def initialize_llm_vllm(model_name, cache_mode='kvtuner', kvtuner_scheme='pertoken', 
    ...
    try:
        ...
        # Check for tqdm-related errors
        elif "tqdm" in error_msg.lower() and "attribute" in error_msg.lower():
            print("\n" + "="*60)
            print("TQDM COMPATIBILITY ERROR DETECTED")
            print("="*60)
            print("This error occurs due to tqdm version incompatibilities in vLLM.")
            print("The issue is likely related to multi-GPU tensor parallelism.")
            print("\nTrying single GPU fallback...")
            print("="*60)
            
            # Try single GPU first
            if llm_kwargs.get("tensor_parallel_size", 1) > 1:
                print("\n🔄 Trying single GPU configuration...")
                try:
                    single_gpu_kwargs = llm_kwargs.copy()
                    single_gpu_kwargs["tensor_parallel_size"] = 1
                    # Remove quantization if it was causing issues
                    if "quantization" in single_gpu_kwargs:
                        single_gpu_kwargs.pop("quantization")
                    ...
                    print("✓ Successfully initialized with single GPU")
                    return llm
                except Exception as fallback_error:
                    print(f"✗ Single GPU fallback failed: {fallback_error}")
